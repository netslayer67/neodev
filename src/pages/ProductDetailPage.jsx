...
const ProductInfo = memo(({
  ...
  const isOutOfStock = selectedSize && selectedSizeStock === 0;
  const canAddToCart = selectedSize && !isOutOfStock;
      ...
      <div className="glass-card p-6 flex flex-col gap-5 shadow-lg">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {product?.sizes?.map(({ size, quantity }) => (
              <button
                ...
                aria-label={`Size ${size}${quantity === 0 ? ' - Out of stock' : ''}`}
              >
                {size}
              </button>
            ))}
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 border border-border rounded-full px-3 py-1 bg-background/60">
            ...
            <Button
              variant="ghost"
              size="icon"
              onClick={() => onQuantityChange(Math.min(quantity + 1, selectedSizeStock))}
        ...
        <Button
          ...
          className={`w-full py-3 font-bold rounded-full transition-all duration-300 flex items-center justify-center gap-2 ${!canAddToCart
              ...
              : "btn-primary hover:opacity-90 hover:scale-105 active:scale-95"
            }`}
        >
          <ShoppingBag size={20} />
          {!selectedSize
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content={product.description?.slice(0, 150) || "Premium streetwear collection"}
        />
        <meta property="og:title" content={`${product.name} — Neo Dervish`} />
      ...
      <div className="min-h-screen pt-20 pb-32 font-sans relative text-foreground overflow-hidden">
        ...
        <div className="absolute -top-40 -left-40 w-96 h-96 bg-accent/15 rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-0 right-0 w-80 h-80 bg-secondary/8 rounded-full blur-3xl animate-pulse" />
              ...
              <div className="mt-8 divide-y divide-border">
                ...
                <AccordionItem title="Details & Fit" icon={Info}>
                  <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                    ...
                    <li>Care: {product.details?.care || "Machine wash cold"}</li>
                  </ul>
                </AccordionItem>
