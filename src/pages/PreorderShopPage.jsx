...
import { Shield, Star, Sparkles, Clock, Award } from "lucide-react";
...
const DecorativeBlobs = memo(() => {
    ...
    if (prefersReducedMotion) {
        return (
            <>
                <div className="absolute -top-32 -left-32 w-80 h-80 bg-gradient-to-br from-warning/15 to-accent/8 rounded-full blur-3xl opacity-60" />
                <div className="absolute top-1/3 -right-24 w-72 h-72 bg-gradient-to-br from-accent/12 to-secondary/8 rounded-full blur-3xl opacity-60" />
                <div className="absolute -bottom-40 left-1/4 w-96 h-96 bg-gradient-to-br from-secondary/8 to-warning/8 rounded-full blur-3xl opacity-60" />
            </>
        );
    }
    ...
    return (
        <>
            <motion.div
                ...
                className="absolute -top-32 -left-32 w-80 h-80 bg-gradient-to-br from-warning/15 to-accent/8 rounded-full blur-3xl"
                style={{ willChange: 'transform' }}
            />
            <motion.div
                animate={{ x: [0, -30, 0], y: [0, 25, 0] }}
                transition={{ duration: 35, repeat: Infinity, ease: "linear", delay: 10 }}
                className="absolute top-1/3 -right-24 w-72 h-72 bg-gradient-to-br from-accent/12 to-secondary/8 rounded-full blur-3xl"
                style={{ willChange: 'transform' }}
            />
            <motion.div
                animate={{ x: [0, 25, 0], y: [0, -15, 0] }}
                transition={{ duration: 40, repeat: Infinity, ease: "linear", delay: 20 }}
                className="absolute -bottom-40 left-1/4 w-96 h-96 bg-gradient-to-br from-secondary/8 to-warning/8 rounded-full blur-3xl"
                style={{ willChange: 'transform' }}
            />
        </>
    );
...
// Optimized Hero Section for Preorder
const HeroSection = memo(() => {
    ...
    return (
        <section className="relative pt-20 pb-8 px-4 sm:px-6 lg:px-8">
            ...
            >
                <div className="inline-flex items-center gap-2 mb-4 px-4 py-2 rounded-full bg-warning/10 border border-warning/20 backdrop-blur-xl">
                ...
                <h1 className="font-heading text-4xl sm:text-5xl lg:text-7xl tracking-tight mb-4">
                    <span className="bg-gradient-to-r from-warning via-accent to-secondary bg-clip-text text-transparent">
                        PREORDER
                    </span>
                    <br />
                ...
                <p className="text-sm sm:text-base text-muted-foreground max-w-md mx-auto mb-6 leading-relaxed">
                    Be the first to own exclusive pieces. Secure your spot before production begins.
                </p>
                ...
                >
                    ...
                    <span className="ml-2 text-xs text-muted-foreground">Exclusive Access</span>
                </motion.div>
            </motion.div>
        </section>
    );
...
// Main PreorderShopPage Component
export default function PreorderShopPage() {
    ...
    // Memoized product list with preorder context
        ...
        <div className="relative min-h-screen bg-gradient-to-br from-background via-background to-card/20 text-foreground overflow-hidden">
            ...
            <main className="px-4 sm:px-6 lg:px-8 py-8">
                <div className="max-w-7xl mx-auto">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-warning/10 to-accent/10 backdrop-blur-md border border-warning/20 rounded-full">
                            <Award className="w-4 h-4 text-warning" />
                            <span className="text-sm font-medium text-foreground">
                                Preorder now for exclusive early access • VA Payment Only
                            </span>
                        </div>
                    </motion.div>
                    ...
                    {status === "succeeded" && products?.length === 0 && (
                        <div className="text-center py-16">
                            <h3 className="text-xl font-heading mb-2">No preorder items available</h3>
                            <p className="text-muted-foreground">Check back soon for new exclusive drops</p>
                        </div>
                    )}
                            ...
                            >
                                <div className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-card/50 to-card/30 backdrop-blur-md border border-border/40 rounded-full">
                                    ...
                                    <span className="text-sm font-medium text-muted-foreground">
                                        You've explored our complete preorder collection
                                    </span>
                                </div>
                            </motion.div>
                        )}
