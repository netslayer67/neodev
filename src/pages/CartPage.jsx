...
// Helper component untuk clean code
...
const CartPage = () => {
  // Menggunakan state agar keranjang dinamis (item bisa dihapus)
  const [cartItems, setCartItems] = useState([
    { ...products[0], quantity: 1 },
    { ...products[2], quantity: 2 },
    { ...products[3], quantity: 1 }, // Menambah item untuk demonstrasi
  ]);

  // Handler untuk mengubah kuantitas atau menghapus item (contoh)
  ...
  const shipping = cartItems.length > 0 ? 15.0 : 0; // Shipping fee premium
  ...
  return (
    ...
    >
      <Helmet>
        <title>Shopping Cart - Radiant Rage</title>
      </Helmet>
      ...
      >
        ...
        {cartItems.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-16 gap-y-12">
            ...
            {/* CART ITEMS LIST - DENGAN ANIMASI */}
                  ...
                  >
                    ...
                    <div className="flex-grow grid grid-cols-3 items-center gap-4">
                      ...
                      <div className="flex items-center justify-center border border-neutral-700 bg-black/20 rounded-full">
                        <IconBtn icon={Minus} onClick={() => handleDecreaseQuantity(item.id)} className="hover:bg-white/10 text-neutral-400 hover:text-white" />
                        <span className="w-10 text-center font-bold text-sm select-none">{item.quantity}</span>
                        <IconBtn icon={Plus} onClick={() => handleIncreaseQuantity(item.id)} className="hover:bg-white/10 text-neutral-400 hover:text-white" />
                      </div>

                      {/* Price & Remove */}
                      <div className="flex items-center justify-end gap-4">
                        <p className="font-semibold text-white text-lg w-24 text-right hidden md:block">
                          ${(item.price * item.quantity).toFixed(2)}
                        </p>
                        <IconBtn icon={X} onClick={() => handleRemoveItem(item.id)} className="text-neutral-500 hover:bg-red-500/20 hover:text-red-400" />
                      </div>
                    </div>
                  </motion.div>
                ))}
            ...
            {/* ORDER SUMMARY - DENGAN EFEK MEWAH */}
            <motion.div
            ...
            >
              <div className="bg-white/5 border border-white/10 backdrop-blur-xl p-8 rounded-2xl shadow-2xl shadow-black/20">
                ...
                <Button
                  asChild
                  size="lg"
                  className="w-full mt-8 bg-white text-black hover:bg-neutral-200 rounded-full font-bold group transition-all duration-300 transform hover:scale-[1.02] focus:scale-[1.02] ring-2 ring-offset-4 ring-offset-black ring-transparent focus:ring-white/50"
                >
          ...
          // EMPTY CART STATE - LEBIH MENARIK
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0, transition: { delay: 0.2 } }}
            className="text-center py-24 flex flex-col items-center"
          >
            <ShoppingCart size={64} className="text-neutral-700 mb-6" />
            ...
            <Button asChild size="lg" className="rounded-full font-bold group">
              <Link to="/shop">
                Continue Shopping
